...
int main(void)
{
	...
	sotype_init(&proj_type, 0);
		...
		*sotype_team(&proj_type) = 1 << 2;
		*sotype_collide(&proj_type) = ~(1 << 2);
	...
	sotype_init(&npc_type, SPACE_OBJ_SHOOT | SPACE_OBJ_TRACK);
		...
		*sotype_damage(&npc_type) = 0;
		projectile_init(sotype_proj(&npc_type), &proj_type, 3.0, 0.7);
